...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (i === 0) {
        // For today, start at the next half hour or 10:00 AM, whichever is later
        if (currentDate.getHours() >= 21) continue; // Skip if it's past 9 PM
        currentDate.setMinutes(currentDate.getMinutes() > 30 ? 30 : 0);
        currentDate.setHours(Math.max(10, currentDate.getHours()));
      } else {
  ...
  const bookAppointment = async () => {
    if (!token) {
      toast.warn("Pehle Login To karle");
    ...
    docInfo && (
      <div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-5">
          <div>
            <img
              className="bg-primary w-80 sm:max-72 rounded-lg"
              src={docInfo.image}
              alt=""
            />
          </div>
          <div className="flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mt-0">
            ...
            <p className="flex items-center gap-2 text-4xl font-medium text-gray-900 mt-5">
              {docInfo.name}
              <img className="w-5" src={assets.verified_icon} alt="" />
            </p>
            <div className="flex items-center gap-2 text-1xl mt-1 text-gray-600 mt-2">
              <p>
